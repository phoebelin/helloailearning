...
- [x] 5.0 Build the flow components and wire them into the page
  ...
  - [x] 5.10 **Drag-to-place coins (updated design).** `play-step.tsx` uses `@dnd-kit/core` (DndContext, DragOverlay, PointerSensor) + `grid-world.tsx` DroppableTile + `coin-tray.tsx` DraggableCoin. Drag coin from tray to grid tile to place; click fallback for keyboard/accessibility; "Re-tune" clears lastRun only (keeps workingReward editable).
  - [x] 5.11 **Drag-to-reposition and drag-off-to-remove placed coins.** `grid-world.tsx` renders a placed coin as a `useDraggable` token (`placed-coin-<id>`, `fromGrid: true` in its drag data) when the grid is in `droppable` mode; `coin-tray.tsx` exposes a `useDroppable` zone (`COIN_TRAY_DROPPABLE_ID`) so dropping a placed coin back on the tray removes it. `play-step.tsx`'s `handleDragEnd` branches on `fromGrid`: drop on a tile moves the coin there (swapping out any occupant), drop on the tray or nowhere removes it. Closes the gap left by 5.10. Verified end-to-end with Playwright (place → reposition → drag-off remove → re-place → run/receipt unaffected).
...
- [ ] 6.0 Course catalog integration, assets, and QA
  ...
  - [x] 6.3 Verify the unlock gate (locked before Activity 3, unlocked after) and that finishing Activity 4 calls `markActivityAsCompleted('how-machines-chase-goals')` and fires `activity-completed`. *(Verified in code: `advanceLevel()` calls `markActivityAsCompleted('how-machines-chase-goals')` at `lib/context/coda-activity-context.tsx:196`; Chapter 4 tile gate wired in PR #19.)*
  - [x] 6.4 Responsive polish (iPad/iPhone): grid scales/scrolls; coin placement + sliders usable by touch; mission card vs grid clearly separated on small screens; tap targets ≥44×44pt. *(Implemented: ResizeObserver-driven tileSize in play-step + mission-step; MouseSensor+TouchSensor for better mobile drag; `p-4 sm:p-8` reduced padding; `min-h-[44px]` on all CTAs; level-complete comparison panel `flex-col sm:flex-row`; slider track `h-3` + `py-2` touch wrapper. Verified 5/5 Playwright tests on 375px mobile.)*
