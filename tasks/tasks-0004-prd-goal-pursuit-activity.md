...
- [x] 1.0 Scaffold the Coda activity: types, context/state machine, and the lesson page shell (mirror Mori)
  ...
  - [x] 1.2 Define the `CodaStep` union in the same file: `meet-coda` → `mission` → `set-reward` → `run` → `receipt` → `level-complete` → `session-summary`, plus a `CodaStepProps` (`onNext` / `onPrevious?`) interface mirroring `MoriStepProps`.
...
- [ ] 6.0 Course catalog integration, assets, and QA
  ...
  - [ ] **6.1a ← DO THIS NEXT — unblock manual testing.** The Chapter 4 tile is in the code but completely invisible due to a layout overflow bug. The chapter row is `flex flex-row` with `p-10` inside a `max-w-[1440px] px-[120px]` page (1200px usable width); four tiles at 261px each + gaps + row padding totals ~1196px — right at the edge — and the page root's `overflow-x-hidden` silently clips the 4th tile. Fix the courses page layout so all four chapter tiles are visible: options include making the chapter row horizontally scrollable (`overflow-x-auto`), reducing its padding for the 4-tile case, or switching to a wrapping/grid layout.
