...
- [ ] 5.0 Build the flow components and wire them into the page
  ...
  - [ ] 5.8 Replace the page placeholders from 1.5 with the real step components; verify the step↔section mapping, the in-place re-tune/re-run loop, the "Next level!" loop, and reset behavior.
  - [ ] 5.9 **Collapse set-reward → run → receipt into one in-place `play` step (updated design).** Create `components/activities/coda/play-step.tsx` that, on a single surface and without scrolling to a new section: shows the `mission-card`, the `grid-world` with the reward controls (`coin-tray` for L1–L2 / `reward-sliders` for L3), a **Run Coda** button that runs `runAgent` and animates Coda on the same grid, and the `receipt-panel` appearing in place after the run. Re-tuning and re-running stay on this surface; only when `matchesTarget` does the child advance to `level-complete`. Update `CodaStep` (1.2) to replace `set-reward`/`run`/`receipt` with a single `play` value, rework the page section mapping (1.5) accordingly (4 sections: `meet-coda` → `mission` → `play` → final), and migrate/retire the now-merged logic from `set-reward-step.tsx` / `run-step.tsx` / `receipt-step.tsx`.
  - [ ] 5.10 **Drag-to-place coins (updated design).** Make coin placement in `coin-tray.tsx` + `grid-world.tsx` a drag-and-drop interaction: drag a coin from the tray onto a grid tile to place it, drag a placed coin to reposition it, and drag it off the grid (or back to the tray) to remove it — all wired to `updateReward`. Keep it usable by touch (HTML5 drag or pointer-events fallback; ≥44pt targets) and within the in-place `play` surface.
