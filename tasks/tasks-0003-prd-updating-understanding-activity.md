...
- [ ] 6.0 Integrate, polish, and QA
  ...
  - [x] 6.5 **Coda continuity teaser (resolves Activity 4 PRD Open Question #7).** `session-summary-step.tsx` now shows a "Next up: Coda" card — Coda's sprite, a short callback line, and a "Meet Coda" CTA to `/lessons/how-machines-chase-goals` — but only once `highestLevelReached >= PIPPY_LEVELS.length` (i.e. the full 3-level arc is actually finished, not a mid-session peek via `exitSession`). Reciprocal to the existing Pippy callback in `components/activities/coda/meet-coda-step.tsx`. Also fixed the Chapter 4 course-tile description in `app/courses/page.tsx`, which read "How machines generate things with Coda" (stale/mismatched copy) → "How machines chase goals with Coda" (matches the actual activity and its PRD title). Note: the courses page has no lock-gating today (removed deliberately in commit `74cedc8`, pending real auth) — this teaser is a narrative nudge, not an unlock mechanism.
